<template>
      ...
      <div class="w-full md:px-12 pt-0">
        <div class="flex flex-wrap md:relative py-4">
          ...
          >
            ...
            <div
              v-if="!isMobile"
              class="p-4 bg-white rounded-xl w-full px-4 mx-auto mb-6"
            >
              <div
  ...
  <div v-if="isLoading">
    <!-- component -->
    <div
      class="inset-0 bg-[#314487A] fixed flex w-full h-full items-center justify-center duration-300 transition-opacity"
      style="z-index: 6000"
    >
      <div class="flex-col">
...
let quizzes = computed(() => quizDetail?.value?.data)
  ? computed(() => quizDetail?.value?.data)
  : null;
let soal = initQuiz(quizzes?.value?.questions ?? []);
// const { questions } = storeToRefs(quizCookies());
// console.log("Soal Answer", soal[selectedSoal.value])
...
function initQuiz(arr: any[]): any[] {
  ...
  if (randomizedArr[0].question.option_c !== null) {
    data.pilihan.push({
      option: "C",
  ...
  if (randomizedArr[0].question.option_d !== null) {
    data.pilihan.push({
      option: "D",
  ...
  if (randomizedArr[0].question.option_e !== null) {
    data.pilihan.push({
      option: "E",
...
const updateVariable = (newValue: number) => {
  try {
    ...
    if (
      soal[selectedSoal.value].question.question_type === "multiple_choice" ||
      answerPicked.value.length !== 0
    ) {
    ...
    if (soal[newValue]?.question) {
      ...
      if (soal[newValue].question.option_c) {
        data.pilihan.push({
          option: "C",
          answer: soal[newValue].question.option_c,
        });
      }
      if (soal[newValue].question.option_d) {
        data.pilihan.push({
          option: "D",
...
const handleTimerFinished = async () => {
  ...
  await useRestClient<APIResponseDetail<QuizScoreResponse>>(
    `/courses/quiz/${quizzes.value?.detail?.session_id}/submit`,
    {
      method: "PATCH",
    }
